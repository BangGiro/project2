...
const fetchExerciseLogs = async (userId, date, setCurrentExercises) => {
  ...
  try {
      const formattedDate = new Date(date).toISOString(); // ISO 8601 형식으로 날짜 포맷
...
function ExerciseMain({ userId }) {  // 부모 컴포넌트에서 userId를 props로 전달받음
    ...
    const handleDateChange = (date) => {
      ...
      const formattedDate = date.toISOString().split('T')[0]; // ISO 형식에서 날짜만 추출 (YYYY-MM-DD)
    ...
    const handleSave = async () => {
      if (userId) {
        const logsToSave = currentExercises.map(exercise => ({
          ...
          reps: exercise.reps,
          sets: exercise.sets,
          date: new Date(selectedDate).toISOString() // ISO 8601 형식으로 변환
      }));
  
          // 배열 형태로 POST 요청 전송
          try {
              await axios.post('/api/exercises/logs', logsToSave, {
                  headers: { 'Content-Type': 'application/json' },
              });
              console.log('운동 기록이 성공적으로 저장되었습니다');
          } catch (error) {
