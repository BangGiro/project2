...
<body>
	...
	<table border="1" style="width: 100%">
		...
		<c:if test="${not empty requestScope.banana}">
			<c:forEach var="m" items="${requestScope.banana}">
				<tr>
					<td><a href="#resultArea2" onclick="idbList('${m.id}')">${m.id}</a></td>
					<!-- ''작은 따옴표 빼먹지 않게 주의 -->
    ...
    -->
					...
					<td>${m.age}</td>
					<td class="textlink"
					 onmouseover="hover(event,'${m.jno}')"
					 onmouseout="hoverout(event,'${m.jno}')">${m.jno}</td>
					<td>${m.info}</td>
					<td>${m.point}</td>
					<td>${m.birthday}</td>
					<td>${m.rid}</td>
        ...
        -->
					...
					<td><span class="textlink"
						onclick="axiDelete(event, '${m.id}')" id="${m.id}">Delete</span></td>
				</tr>
			</c:forEach>
		</c:if>
