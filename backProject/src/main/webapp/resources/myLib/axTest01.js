...
//=> 성공/실패 여부만 전달 받으면 됨
//-> Controller: page를 response에 담는 것이 아니고, 결과 값만 담으면 됨
//-> 위와 같은 기능을 처리하는 전용 Controller를 RestController라함
//=> Ajax요청 fetch적용

//=> server: @RestController, 계층적 uri 적용 , post요청
...
function rsLogin(){
	...
	fetch(url,{
		...
		if (err.message=='502') alert(`id 또는 password 오류입니다. 다시하세요`)
...
function rsLoginjj(){
	...
	fetch(url,{
		...
		headers: {'Content-Type':'application/json'}
				...
				//=> 서버에서JSON 형식으로 보냈으므로 json()메서드 사용
...
function axiLoginjj(){
	...
	axios({url:url,
		...
		location.reload();
			...
			if(err.response.status=='502')alert(`id 또는 password오류입니다 다시하세요`)
			else alert(`System 오류입니다. 잠시 후 다시하세요 status=> ${err.message}`)
		});
